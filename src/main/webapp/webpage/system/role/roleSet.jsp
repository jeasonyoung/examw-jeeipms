...
<script type="text/javascript">
	$(function() {
		$('#functionid').tree({
			checkbox : true,
			url : 'sysroleController.do?setAuthority&roleId=${roleId}',
			...
			onClick: function(node){
				...
				if(isRoot==''){
					var roleId = $("#rid").val();
					$('#operationListpanel').panel("refresh", "sysroleController.do?operationListForFunction&functionId="+node.id+"&roleId="+roleId);
				}else {
				}
			}
		});
	...
	function mysubmit() {
		var roleId = $("#rid").val();
		var s = GetNode();
		doSubmit("sysroleController.do?updateAuthority&rolefunctions=" + s + "&roleId=" + roleId);
	}
	function GetNode() {
