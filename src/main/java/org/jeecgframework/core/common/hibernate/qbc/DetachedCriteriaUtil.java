...
public class DetachedCriteriaUtil {
	 ...
	 */
	public static DetachedCriteria createDetachedCriteria(Class<?> entityClazz,
			String startChar,String alias) {
		return createDetachedCriteria(entityClazz, startChar, alias,null);
	}
	...
	public static DetachedCriteria createDetachedCriteria(Class<?> entityClazz,
		...
		return createDetachedCriteria(entityClazz, startChar, alias, columnNames, null);
	}
	...
	public static DetachedCriteria createDetachedCriteria(Class<?> entityClazz,
		...
		DetachedCriteria criteria = DetachedCriteria.forClass(entityClazz,alias);
		if(columnNames!=null && columnNames.length>0){
			selectColumn(criteria, columnNames, entityClazz, false);
		}
		return criteria;
	}
	...
	public static void selectColumn(DetachedCriteria criteria, String[] columnNames,
			Class<?> entityClass,boolean forJoinTable) {
		if (null == columnNames) {
			return;
		}
		...
		if(!hasJoniTable){
			...
			criteria.setResultTransformer(new AliasToBean(entityClass));
		}
	}
