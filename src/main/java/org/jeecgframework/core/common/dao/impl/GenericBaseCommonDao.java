...
public abstract class GenericBaseCommonDao<T, PK extends Serializable> implements IGenericBaseCommonDao {
	 ...
	 * @return
	 */
	public <T>List<T> getListByOrder(Class<T> entityClass,String propertyName,Object value, String ordername, boolean isAsc,String method){
		if(method.toUpperCase().equals("ISNOTNULL")){
			return createCriteria(entityClass,ordername, isAsc,Restrictions.isNotNull(propertyName)).list();
		}
		if(method.toUpperCase().equals("ISNULL")){
			return createCriteria(entityClass,ordername, isAsc,Restrictions.isNull(propertyName)).list();
		}
		if(method.toUpperCase().equals("EQ")){
			return createCriteria(entityClass,ordername, isAsc,Restrictions.eq(propertyName, value)).list();
		}
		if(method.toUpperCase().equals("NE")){
			return createCriteria(entityClass,ordername, isAsc,Restrictions.ne(propertyName, value)).list();
		}else{
			return createCriteria(entityClass,ordername, isAsc).list();
		}
