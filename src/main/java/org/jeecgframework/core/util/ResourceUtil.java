...
public class ResourceUtil {
	...
	public static final AgencyUserEntity  getSessionAgencyUser() {
		HttpSession session = ContextHolderUtils.getSession();
		session.setMaxInactiveInterval(-1);
		if (session.getAttributeNames().hasMoreElements()) {
			return  (AgencyUserEntity) session.getAttribute(Globals.AGENCY_USER_SESSION);
		} else {
			return null;
		}
	}
	
	public static final AgencyEntity getAgency(){
		HttpSession session = ContextHolderUtils.getSession();
		session.setMaxInactiveInterval(-1);
		if (session.getAttributeNames().hasMoreElements()) {
			return (AgencyEntity) session.getAttribute(Globals.AGENCY_SESSION);
		} else {
			return null;
		}
	}
	...
	public static final String getAgencyId(){
		...
		session.setMaxInactiveInterval(-1);
		if (session.getAttributeNames().hasMoreElements()) {
			return (String) session.getAttribute("agengcyid");
		} else {
			return null;
		}
	}
	
	public static final StudentEntity getStudent(){
		HttpSession session = ContextHolderUtils.getSession();
		session.setMaxInactiveInterval(-1);
		if (session.getAttributeNames().hasMoreElements()) {
			return (StudentEntity) session.getAttribute("student");
		} else {
			return null;
		}
	}
