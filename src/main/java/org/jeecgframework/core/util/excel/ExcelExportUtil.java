...
public class ExcelExportUtil {
	 ...
	 * 
	 * @param title     Sheet名字
	 * @param entityClass Excel对象Class
	 * @param dataSet   Excel对象数据List
	 * @param out       输出流
	 */
	public static void exportExcel(String title, Class<?> entityClass,
			Collection<?> dataSet, OutputStream out) {
		// 使用userModel模式实现的，当excel文档出现10万级别的大数据文件可能导致OOM内存溢出
		exportExcelInUserModel(title, entityClass, dataSet, out);
		// 使用eventModel实现，可以一边读一边处理，效率较高，但是实现复杂，暂时未实现
	}

	private static void exportExcelInUserModel(String title, Class<?> entityClass,
			Collection<?> dataSet, OutputStream out) {
		try {
			// 首先检查数据看是否是正确的
			...
			if (title == null || out == null || entityClass == null) {
			...
			for (int i = 0; i < fileds.length; i++) {
				...
				if (excel != null) {
					...
					if (excel.exportConvertSign() == 1) {
						...
						Method getConvertMethod = entityClass
	 ...
	 * @param out       输出流
	 */
	public static HSSFWorkbook  exportExcel(String title, Class<?> entityClass,
			Collection<?> dataSet) {
		// 使用userModel模式实现的，当excel文档出现10万级别的大数据文件可能导致OOM内存溢出
		return exportExcelInUserModel2File(title, entityClass, dataSet);
	}
	
	
	
	@SuppressWarnings("unchecked")
	private static HSSFWorkbook exportExcelInUserModel2File(String title, Class<?> entityClass,
			Collection<?> dataSet) {
		// 声明一个工作薄
...
//				throw new Exception("导出数据为空！");
//			}
//			if (title == null || entityClass == null) {
//				throw new Exception("传入参数不能为空！");
//			}
			// 声明一个工作薄
